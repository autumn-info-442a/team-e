...
79.	The system must allow the user to access the Admin Panel tab on the My Groups page, giving access to the user’s Admin groups and pending group requests. 
80.	For each group the user is an admin in, the system must display the name of each user pending.
81.	The system must allow the Admin user to click on a pending request to make the decision to admit them.
82.	The system must display a confirmation when the Admin user clicks on a pending request, allowing them to click yes or no.
83.	The system must allow the pending user to access the group if they are admitted by setting the user as a group member or cancel the request if they are denied.
84.	The system must be able to recognize when a user is an admin for a group (a group they created), giving them the ability to delete the group if desired from the “Your Admin Groups” tab by hitting the delete button.
85.	The system must display a delete confirmation screen, allowing the user to delete the group by clicking yes, or no to not delete the group.
86.	The system must remove the pending request from the Admin user’s view after a decision has been made.
87.	The system must display new groups requests in My Groups > Admin Groups so that the group Admin can admit or decline the pending member. 
